...
Historically, the app used an in-memory `deque` for the claim queue in Phase 1. Multiple
uvicorn workers would each have had their own copy of that queue — the same problem
that originally blocked multi-replica deployments. Phase 2 moved OTP/admin runtime
state to Redis. The image still runs one worker by default so that worker-count changes
are validated deliberately through the same OTP flow testing used for app replica changes.
